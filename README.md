...
- Active product direction: Autonomous Release Steward checklist Items 1–9 are merged to `main`; Item 10's live Bedrock gate passed and the fail-closed `/ready` contract is implemented locally, while deployment, webhook, worker, and credential-install gates remain on HOLD
...
- Hosting target: Railway is live in guarded `public-read-only` mode at `quietops-production.up.railway.app`. A bounded 2026-08-26 KST observation found `/health` `200`, `/api/release-runs` `200`, `/ready` `404`, and a release marker still naming predecessor commit `8c4c7421aef135541bc16294b017daae5515aa33`; see the [runtime readiness gate](docs/RUNTIME_READINESS_GATE_2026-08-26.md).
...
- Deployment: the previously recorded Railway deployment and current public release marker are historical predecessor evidence, not proof of the current successor. The next deployment remains `HOLD_DEPLOY` until the Railway CLI is authenticated to the owner-approved account.
