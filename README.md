...
Authenticated release decisions are also absent by default. Setting a 32-256 byte `QUIETOPS_OPERATOR_TOKEN` without whitespace or control characters registers `POST /api/decisions/:decisionId`. The route validates `Authorization: Bearer <token>` in constant time before parsing the decision contract, requires a bounded `Idempotency-Key`, and accepts the construction-bound `WAIT_AND_RECHECK` and `ESCALATE_INCIDENT` choices. Escalation stores the authorization, stable SHA-256 request fingerprint, and `RESERVED` action before the worker can mark one attempt `IN_FLIGHT`. A valid GitHub `201` confirms; `4xx` rejects; timeout, connection loss, `5xx`, malformed success, or a crash around the provider call becomes terminal `UNCERTAIN` with no automatic retry. Item 8 tests use only injected provider doubles: no GitHub token was installed and no live network write occurred. Installing operator/provider credentials, exposing this successor route publicly, or performing the first real issue POST remains a separate operational gate.
