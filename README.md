...
- Active increment: Stage 4C-1c — an exact-commit, no-store release-marker route plus a fail-closed public-bind requirement; the broader Stage 1, Stage 2, Stage 4, and Stage 5 plans remain incomplete
...
- Hosting target: Railway is selected as `PREPARE_ONLY`; public-write, host/`PORT`, liveness, and release-marker blockers are closed locally, but persistent-volume and production-start work remain, and no Railway resource has been created
