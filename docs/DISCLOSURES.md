...
This public repository contains planning documents, an MIT license, a TypeScript contract kernel, newly authored credential-free Ready/mismatch Strands agent slices, an append-only SQLite application spine, and a local Fastify/browser decision workflow. The mismatch slice also includes an optional Bedrock model-selection path. The published browser claim is limited to the verified loopback demo; live-provider, hosted deployment, and live AWS/Bedrock claims remain absent.
...
The contract kernel uses TypeScript and Prettier as development tools plus the Node.js type definitions. The mismatch slice uses the Strands Agents SDK and Zod. Its Bedrock path uses `BedrockModel`, already included in the pinned Strands SDK, and the AWS SDK default credential chain; no credential values are read or printed by QuietOps. The application spine uses the existing Node.js runtime's built-in `node:sqlite` API and adds no database package. The local HTTP slice adds the exact Fastify runtime recorded in the provenance ledger and repository-authored HTML, CSS, JavaScript, and SVG. Playwright CLI was used as an ephemeral local browser-verification tool and is not an application dependency. React and AgentCore remain unintroduced.
