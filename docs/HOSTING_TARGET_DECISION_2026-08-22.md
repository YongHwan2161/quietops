...
5. Closed locally by Stages 4C-1e and 4C-1f: root `npm start` executes only prebuilt output, and `railway.json` fixes the Railpack build, start, and health-check contract.
6. Still external: the service must receive the explicit public-mode variables and an actual volume mount; neither exists or is represented as verified.
...
- use the deterministic production start command (`COMPLETE_LOCAL`, Stage 4C-1e) and schema-bound Railway configuration (`COMPLETE_LOCAL`, Stage 4C-1f);
