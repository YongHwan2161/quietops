...
- The credential-free Ready/mismatch judge path, append-only evaluation/application spine, loopback-only HTTP/browser decision workflow, and a bounded public-GitHub source/CI adapter are implemented. The GitHub adapter is not yet wired into the Strands evaluation or product workflow. SSE, background orchestration, deployment/browser collectors, export, authentication, Docker packaging, architecture assets, and public video remain incomplete.
